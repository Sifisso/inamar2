...
public class TelinhaCtrl extends GenericForwardComposer{
	...
	public void onClick$btn_gravar(){
		if(_peticao!=null){
			saveOrUpdate(_pagamento, _peticao);
			Clients.showNotification("Petição validada com sucesso", "info", txb_observacoes, "before_center", 4000, true);
	...
	private void saveOrUpdate(Pagamento pag, Peticao _petic) {
		
		_petic.setValidado(true);
		_petic.setAdmMaritima(true);
		_petic.setRecusado(false);
		
		_petic.setChefeSecretaria(false);
		_petic.setLocalizacao("Administrador Maritimo");
		_peticaoService.update(_petic);
