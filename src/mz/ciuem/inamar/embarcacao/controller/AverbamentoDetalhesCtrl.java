...
public class AverbamentoDetalhesCtrl extends GenericForwardComposer{
	

	private Window win_averbamento;
	...
	private Textbox tbx_marcaMotor, tbx_nrMotor;
...
//		}
		...
		private void gravar() {
			_peticaoEmbarcacao.setNrMotor(tbx_nrMotor.getValue());
			_peticaoEmbarcacao.setMarcaMotor(tbx_marcaMotor.getValue());
		...
		private void verFactura() throws JRException {
		       ...
		       List<PeticaoMaritimoTaxaPedido> _list = _peticaoMaritimoTaxaPedidoService.findByPeticaoEmbarcacao(_peticaoEmbarcacao);
		   	   MasterRep.imprimir("/reportParam/facturaPedidoUtente.jrxml", _list, mapaParam, win_averbamento);
		...
		private void preencherCampos() {
			if(_peticaoEmbarcacao!=null && _peticaoEmbarcacao.getPeticao()!=null){
				//_peticaoEmbarcacao = _peticaoEmbarcacaoService.findOneWithEager(_peticaoEmbarcacao.getId());
					tbx_marcaMotor.setValue(_peticaoEmbarcacao.getMarcaMotor());
					tbx_nrMotor.setValue(_peticaoEmbarcacao.getNrMotor());
