...
public class TransferenciaDetalhesCtrl extends GenericForwardComposer{
	

	private Window win_transferencia;
	...
	private Textbox tbx_denominacao, tbx_proprietario, tbx_matricula, tbx_transferencia, tbx_duracao, tbx_finalidade;
...
//		}
		...
		private void gravar() {
			...
			_peticaoEmbarcacao.setTranferencia(tbx_transferencia.getValue());
			_peticaoEmbarcacao.setDuracao(tbx_duracao.getValue());
			_peticaoEmbarcacao.setFinalidade(tbx_finalidade.getValue());
			_peticaoEmbarcacao.setEhTemporario(rdx_temporaria.isChecked() ? true : false);
			_peticaoEmbarcacao.setPais((Pais) cbx_pais.getSelectedItem().getValue());
		...
		private void verFactura() throws JRException {
		       ...
		       List<PeticaoMaritimoTaxaPedido> _list = _peticaoMaritimoTaxaPedidoService.findByPeticaoEmbarcacao(_peticaoEmbarcacao);
		   	   MasterRep.imprimir("/reportParam/facturaPedidoUtente.jrxml", _list, mapaParam, win_transferencia);
...
//		}
		...
		private void preencherCampos() {
			if(_peticaoEmbarcacao!=null && _peticaoEmbarcacao.getPais()!=null){
				_peticaoEmbarcacao = _peticaoEmbarcacaoService.findOneWithEager(_peticaoEmbarcacao.getId());
					...
					tbx_transferencia.setValue(_peticaoEmbarcacao.getTranferencia());
					tbx_finalidade.setValue(_peticaoEmbarcacao.getFinalidade());
					tbx_denominacao.setValue(_peticaoEmbarcacao.getDenominacao());
					tbx_duracao.setValue(_peticaoEmbarcacao.getDuracao());
					cbx_pais.setValue(_peticaoEmbarcacao.getPais()==null ? "" : _peticaoEmbarcacao.getPais().getDesignacao());
		...
		private void listarPais() {
			List<Pais> listPais = _paisService.getAll();
			cbx_pais.setModel(new ListModelList<Pais>(listPais));
		}
