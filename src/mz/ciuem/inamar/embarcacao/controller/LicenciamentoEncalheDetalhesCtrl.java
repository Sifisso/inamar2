...
public class LicenciamentoEncalheDetalhesCtrl extends GenericForwardComposer{
	

	private Window win_licenciamentoEncalhe;
	private Div div_content_out, div_terminar, div_utente, div_secretario, myModal, div_dados;
	private Include inc_main;
	private Combobox cbx_pais, cbx_classeEmbarcacao, cbx_servico, cbx_local;
	...
	private Textbox tbx_denominacao, tbx_proprietario, tbx_nrMpt;
...
//		}
		...
		private void gravar() {
			...
			_peticaoEmbarcacao.setNrMpt(tbx_nrMpt.getValue());
		...
		private void verFactura() throws JRException {
		       ...
		       List<PeticaoMaritimoTaxaPedido> _list = _peticaoMaritimoTaxaPedidoService.findByPeticaoEmbarcacao(_peticaoEmbarcacao);
		   	   MasterRep.imprimir("/reportParam/facturaPedidoUtente.jrxml", _list, mapaParam, win_licenciamentoEncalhe);
		...
		public void onClick$btn_prevalidar(){
			if(_peticaoEmbarcacao!=null && _peticaoEmbarcacao.getPeticao()!=null){
				...
				win_licenciamentoEncalhe.detach();
			}
		...
		private void preencherCampos() {
			if(_peticaoEmbarcacao!=null && _peticaoEmbarcacao.getPais()!=null){
				_peticaoEmbarcacao = _peticaoEmbarcacaoService.findOneWithEager(_peticaoEmbarcacao.getId());
					...
					tbx_nrMpt.setValue(_peticaoEmbarcacao.getNrMpt());
