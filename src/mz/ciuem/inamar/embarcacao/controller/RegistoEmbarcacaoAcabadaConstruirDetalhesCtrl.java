...
public class RegistoEmbarcacaoAcabadaConstruirDetalhesCtrl extends GenericForwardComposer{
	
	private Window win_embarcacaoAcabadaConstruir;
	private Div div_content_out, div_terminar, div_utente, div_secretario, myModal, div_dados;
	private Include inc_main;
	private Combobox cbx_classeEmbarcacao, cbx_servico, cbx_local;
	private Button btn_proximo, btn_imprimir, btn_terminar, btn_voltarUtente, btn_finalizar, btn_validar, btn_recusar, btn_prevalidar;
	private Textbox tbx_denominacao;
	private Doublebox dbx_metrosc,dbx_metrosb, dbx_metrosp;
	private Label lbl_nome, lbl_idade, lbl_contacto, lbl_provincia, lbl_distrito, lbl_tipoDoc, lbl_nrDoc, lbl_bairro, lbl_localEmissao;
	...
	private void pegarPeticaoEmbarcacao() {
		 ...
		 private void ocultarCampos() {
			...
			private void gravar() {
				_peticaoEmbarcacao.setLocalConstrucao(cbx_local.getValue());
				_peticaoEmbarcacao.setClasseEmbarcacao((ClasseEmbarcacao) cbx_classeEmbarcacao.getSelectedItem().getValue());
				_peticaoEmbarcacao.setServicoDestino((ServicoDestino) cbx_servico.getSelectedItem().getValue());
				_peticaoEmbarcacao.setBoca(dbx_metrosb.getValue());
				_peticaoEmbarcacao.setComprimento(dbx_metrosc.getValue());
				_peticaoEmbarcacao.setPontal(dbx_metrosp.getValue());
			...
			private void verFactura() throws JRException {
			       ...
			       List<PeticaoMaritimoTaxaPedido> _list = _peticaoMaritimoTaxaPedidoService.findByPeticaoEmbarcacao(_peticaoEmbarcacao);
			   	   MasterRep.imprimir("/reportParam/facturaPedidoUtente.jrxml", _list, mapaParam, win_embarcacaoAcabadaConstruir);
			...
			public void onClick$btn_prevalidar(){
				if(_peticaoEmbarcacao!=null && _peticaoEmbarcacao.getPeticao()!=null){
					...
					showNotifications("Petição pré-validada Registada com sucesso.", "info");
				}
			...
			private void preencherCampos() {
				if(_peticaoEmbarcacao!=null && _peticaoEmbarcacao.getClasseEmbarcacao()!=null){
				    _peticaoEmbarcacao = _peticaoEmbarcacaoService.findOneWithEager(_peticaoEmbarcacao.getId());
					cbx_local.setValue(_peticaoEmbarcacao.getLocalConstrucao());
					cbx_classeEmbarcacao.setValue(_peticaoEmbarcacao.getClasseEmbarcacao()==null ? "" : _peticaoEmbarcacao.getClasseEmbarcacao().getNome());
					dbx_metrosb.setValue(_peticaoEmbarcacao.getBoca());
					dbx_metrosc.setValue(_peticaoEmbarcacao.getComprimento());
					dbx_metrosp.setValue(_peticaoEmbarcacao.getPontal());
					...
					lbl_nome.setValue(_peticaoEmbarcacao.getPeticao().getUtente());
					lbl_contacto.setValue(_peticaoEmbarcacao.getPeticao().getContactoUtente());
					lbl_provincia.setValue(_peticaoEmbarcacao.getPeticao().getProvincia());
					lbl_distrito.setValue(_peticaoEmbarcacao.getPeticao().getDistritoUtente());
					lbl_tipoDoc.setValue(_peticaoEmbarcacao.getPeticao().getTipDocUtente());
					lbl_nrDoc.setValue(_peticaoEmbarcacao.getPeticao().getNrDocUtente());
					lbl_bairro.setValue(_peticaoEmbarcacao.getPeticao().getDataNascUtente());
					lbl_localEmissao.setValue(_peticaoEmbarcacao.getPeticao().getLocalEmissaoUtente());
				}
			}
			...
			private void preencherClasseEmbarcacao() {
					List<ClasseEmbarcacao> listClasseEmbarcacao = _classeEmbarcacaoService.getAll();
					cbx_classeEmbarcacao.setModel(new ListModelList<ClasseEmbarcacao>(listClasseEmbarcacao));
				}
