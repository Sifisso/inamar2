...
public class EmissaoLicencaEstivaDetlhesCtrl extends GenericForwardComposer{
	
	private Window win_emissaoLicEstivaDet;
	...
	private Textbox tbx_user, tbx_pass, tbx_nomeEmpresa, tbx_endereco, tbx_contacto, tbx_email, tbx_nuit;
	...
	private void listarActividades(){
		List<ActividadeLicenca> listActividadesLicenca = _actividadeLicencaService.findActividadesByActivo();
		lbx_actividades.setModel(new ListModelList<ActividadeLicenca>(listActividadesLicenca));
	}
	
	private void listarLocalPratica(){
		List<LocalPratica> listLocalPratica = _localPraticaService.findLocalPraticaByActivo();
		lbx_locais.setModel(new ListModelList<LocalPratica>(listLocalPratica));
	...
	private void preencherCampos() {
		if(_peticaoLicenca!=null && _peticaoLicenca.getPeticao()!=null){
			...
			lbl_nome.setValue(_peticaoLicenca.getPeticao().getUtente());
			lbl_contacto.setValue(_peticaoLicenca.getPeticao().getContactoUtente());
			lbl_provincia.setValue(_peticaoLicenca.getPeticao().getProvincia());
			lbl_distrito.setValue(_peticaoLicenca.getPeticao().getDistritoUtente());
			lbl_tipoDoc.setValue(_peticaoLicenca.getPeticao().getTipDocUtente());
			lbl_nrDoc.setValue(_peticaoLicenca.getPeticao().getNrDocUtente());
			lbl_bairro.setValue(_peticaoLicenca.getPeticao().getDataNascUtente());
			lbl_localEmissao.setValue(_peticaoLicenca.getPeticao().getLocalEmissaoUtente());
		}
	}
	...
	private void gravar() {
		...
		_peticaoLicenca.setActividadeLicenca(cbx_actividadeLicenca.getSelectedItem().getValue());
		_peticaoLicenca.setLocalPratica(cbx_localPratica.getSelectedItem().getValue());
